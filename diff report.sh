...
diff_packages() {
    ...
    grep "|" "$pre_f"  2>/dev/null | grep -v "^=" | grep -v "^Total" | sort > /tmp/patch_pre_pkgs.txt
    grep "|" "$post_f" 2>/dev/null | grep -v "^=" | grep -v "^Total" | sort > /tmp/patch_post_pkgs.txt

    # Get package names only for comparison
    awk -F'|' '{print $1}' /tmp/patch_pre_pkgs.txt  | sort -u > /tmp/patch_pre_names.txt
    awk -F'|' '{print $1}' /tmp/patch_post_pkgs.txt | sort -u > /tmp/patch_post_names.txt

    # Newly installed
    PKG_NEW=$(comm -13 /tmp/patch_pre_names.txt /tmp/patch_post_names.txt 2>/dev/null)
    PKG_NEW_COUNT=$(echo "$PKG_NEW" | grep -c . 2>/dev/null || echo 0)

    # Removed packages
    PKG_REMOVED=$(comm -23 /tmp/patch_pre_names.txt /tmp/patch_post_names.txt 2>/dev/null)
    PKG_REMOVED_COUNT=$(echo "$PKG_REMOVED" | grep -c . 2>/dev/null || echo 0)

    # Updated (same name, different version)
    PKG_UPDATED=""
    while IFS= read -r pkg; do
        pre_line=$(grep "^${pkg}|" /tmp/patch_pre_pkgs.txt  2>/dev/null | head -1)
        post_line=$(grep "^${pkg}|" /tmp/patch_post_pkgs.txt 2>/dev/null | head -1)
    ...
    rm -f /tmp/patch_pre_pkgs.txt /tmp/patch_post_pkgs.txt /tmp/patch_pre_names.txt /tmp/patch_post_names.txt
}
...
diff_services() {
    ...
    grep "running" "$pre_f"  2>/dev/null | awk '{print $1}' | sort > /tmp/patch_pre_svc.txt
    grep "running" "$post_f" 2>/dev/null | awk '{print $1}' | sort > /tmp/patch_post_svc.txt

    SVC_NEW=$(comm -13 /tmp/patch_pre_svc.txt /tmp/patch_post_svc.txt 2>/dev/null)
    SVC_NEW_COUNT=$(echo "$SVC_NEW" | grep -c "\." 2>/dev/null || echo 0)
    SVC_STOPPED=$(comm -23 /tmp/patch_pre_svc.txt /tmp/patch_post_svc.txt 2>/dev/null)
    ...
    rm -f /tmp/patch_pre_svc.txt /tmp/patch_post_svc.txt
}
...
diff_ports() {
    ...
    grep "LISTEN" "$pre_f"  2>/dev/null | awk '{print $NF, $(NF-1)}' | sort > /tmp/patch_pre_ports.txt
    grep "LISTEN" "$post_f" 2>/dev/null | awk '{print $NF, $(NF-1)}' | sort > /tmp/patch_post_ports.txt

    PORTS_NEW=$(comm -13 /tmp/patch_pre_ports.txt /tmp/patch_post_ports.txt 2>/dev/null)
    PORTS_CLOSED=$(comm -23 /tmp/patch_pre_ports.txt /tmp/patch_post_ports.txt 2>/dev/null)
    ...
    rm -f /tmp/patch_pre_ports.txt /tmp/patch_post_ports.txt
}
...
diff_etc() {
    ...
    grep "^[a-f0-9]" "$pre_f"  2>/dev/null | sort > /tmp/patch_pre_etc.txt
    grep "^[a-f0-9]" "$post_f" 2>/dev/null | sort > /tmp/patch_post_etc.txt

    # Files with changed checksums
    awk '{print $2, $1}' /tmp/patch_pre_etc.txt  | sort > /tmp/patch_pre_etc_inv.txt
    awk '{print $2, $1}' /tmp/patch_post_etc.txt | sort > /tmp/patch_post_etc_inv.txt

    ETC_MODIFIED=$(join -j 1 /tmp/patch_pre_etc_inv.txt /tmp/patch_post_etc_inv.txt 2>/dev/null | awk '$2 != $3 {print $1}' | head -50)
    ETC_NEW=$(comm -13 <(awk '{print $1}' /tmp/patch_pre_etc_inv.txt | sort) <(awk '{print $1}' /tmp/patch_post_etc_inv.txt | sort) 2>/dev/null | head -30)
    ETC_REMOVED=$(comm -23 <(awk '{print $1}' /tmp/patch_pre_etc_inv.txt | sort) <(awk '{print $1}' /tmp/patch_post_etc_inv.txt | sort) 2>/dev/null | head -30)
    ...
    rm -f /tmp/patch_pre_etc.txt /tmp/patch_post_etc.txt /tmp/patch_pre_etc_inv.txt /tmp/patch_post_etc_inv.txt
}
...
diff_disk() {
    ...
    grep "^/" "$pre_f"  2>/dev/null | awk '{print $1,$5}' | sort > /tmp/patch_pre_disk.txt
    grep "^/" "$post_f" 2>/dev/null | awk '{print $1,$5}' | sort > /tmp/patch_post_disk.txt
    DISK_DIFF=$(diff /tmp/patch_pre_disk.txt /tmp/patch_post_disk.txt 2>/dev/null)
    if [ -n "$DISK_DIFF" ]; then
        print_check "Disk Usage" "CHANGED" "mount point usage changed"
    else
        print_check "Disk Usage" "SAME" "no changes"
    fi
    rm -f /tmp/patch_pre_disk.txt /tmp/patch_post_disk.txt
}
...
diff_users() {
    ...
    grep "^.*:.*:.*:.*:.*:.*:" "$pre_f"  2>/dev/null | grep -v "^#" | sort > /tmp/patch_pre_users.txt
    grep "^.*:.*:.*:.*:.*:.*:" "$post_f" 2>/dev/null | grep -v "^#" | sort > /tmp/patch_post_users.txt
    USERS_NEW=$(comm -13 /tmp/patch_pre_users.txt /tmp/patch_post_users.txt 2>/dev/null | awk -F: '{print $1}' | tr '\n' ' ')
    USERS_REM=$(comm -23 /tmp/patch_pre_users.txt /tmp/patch_post_users.txt 2>/dev/null | awk -F: '{print $1}' | tr '\n' ' ')
    ...
    rm -f /tmp/patch_pre_users.txt /tmp/patch_post_users.txt
}
